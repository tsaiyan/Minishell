...
int	it_not_redirect_str(char *str, int len)
{
	if (len)
	{
		if (!ft_strncmp(str, "\007", len))
			return (0);
		if (!ft_strncmp(str, "\007\007", len))
			return (0);
		if (!ft_strncmp(str, "\006", len))
			return (0);
	}
	if (len == 0)
	{	
...
void	make_new_str(t_bin *bin, int i, int j)
{
	...
	if (it_not_redirect_str(bin->argv[i], red_len))
	{
...
void	make_redirects(t_bin *bin)
{
	...
	while (flag)
	{
		...
		{
			if (!it_not_redirect_str(bin->argv[i], 0))
				continue ;
			j = -1;
